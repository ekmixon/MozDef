...
def fetch_ip_list(aws_key_id, aws_secret_key, s3_bucket, ip_list_filename):
    ...
    return [ip.decode() for ip in ip_content_list]
...
def save_ip_list(save_path, ips):
    ...
    if os.path.isfile(save_path):
        logger.debug(f"Overwriting ip list file in {str(save_path)}")
    else:
        logger.debug(f"Creating new ip list file at {str(save_path)}")
...
def main():
    ...
    for manual_addition in options.manual_additions:
        if manual_addition == '':
            continue
        logger.debug(f"Adding manual addition: {manual_addition}")
        ips.append(manual_addition)

    if len(ips) < options.ips_list_threshold:
        raise LookupError(
            f'IP List contains less than {str(options.ips_list_threshold)} entries...something is probably up here.'
        )
